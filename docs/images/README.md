...
> Show Agent Tool Request → Validate Input + Enforce ACL → Toolbox → Structured Observation → Back to Agent. The toolbox contains Search Docs/Qdrant, Get Record/PostgreSQL, Graph Traverse/Neo4j, Schedule Impact/Schedule Service, Compare Revisions/Source Files, and Calculator/No LLM. Observations return result data, source IDs, citations, confidence/errors, and elapsed time. Mark the MVP read-only. Future writes require preview, human approval, and an audit log. Emphasize: “Agents decide what to do. Tools do the work.”
